...
const handlerWithBodyType = handleRequest<
  { hello: string },
  { parseBody: true }
>({ parseBody: true }, async (options) => {
  // `options.parsedRequestBody` is of type `{ hello: string }` because all generic paramters are being used.
...
const handlerWithAuth = handleRequest(
  ...
  async (options) => {
    // `options` contains `request` and `response` and `userId` because no generic paramter is being used.
    const expandedOptions: Expand<typeof options> = options
    //    ^?
  }
)
